...
var (
	...
	flagTimeout  = flag.Duration("timeout", 600*time.Second, "Per-request timeout")
...
func handleSkills(runtime *agentRuntime) http.HandlerFunc {
	return func(w http.ResponseWriter, _ *http.Request) {
		...
		writeJSON(w, 200, map[string]any{"skills": ag.WebUISkills()})
	}
}
...
func utcpToolSpec(tool tools.Tool) agent.ToolSpec {
	...
	if len(tool.Inputs.Required) > 0 {
		schema["required"] = tool.Inputs.Required
	}
	if tool.Inputs.Description != "" {
		schema["description"] = tool.Inputs.Description
	}
	if tool.Inputs.Title != "" {
		schema["title"] = tool.Inputs.Title
	}
	if tool.Inputs.Items != nil {
		schema["items"] = tool.Inputs.Items
	}
	if len(tool.Inputs.Enum) > 0 {
		schema["enum"] = tool.Inputs.Enum
	}
	if tool.Inputs.Format != "" {
		schema["format"] = tool.Inputs.Format
	}
	if tool.Inputs.Minimum != nil {
		schema["minimum"] = *tool.Inputs.Minimum
	}
	if tool.Inputs.Maximum != nil {
		schema["maximum"] = *tool.Inputs.Maximum
	}
...
func handleCreateSubagent(runtime *agentRuntime) http.HandlerFunc {
	return func(w http.ResponseWriter, req *http.Request) {
		...
		if strings.ContainsAny(body.Name, " .:/\\") {
			writeError(w, 400, "name may only contain letters, numbers, '-' and '_'")
			return
		}
		if body.Description == "" {
			body.Description = "Sub-agent created from the go-agent WebUI"
		}
		if body.SystemPrompt == "" {
			body.SystemPrompt = body.Description
		}
		runtime.mu.Lock()
		if _, exists := runtime.subagents[body.Name]; exists {
			runtime.mu.Unlock()
			writeError(w, 409, "sub-agent already exists")
			return
		}
		...
		if err != nil {
			writeError(w, 400, fmt.Sprintf("create model: %v", err))
			return
		}
...
func buildAgent(ctx context.Context) (*agent.Agent, utcp.UtcpClientInterface, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("create model (%s): %w", *flagProvider, err)
	}
...
func handleStream(runtime *agentRuntime) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !ok {
			writeError(w, 500, "streaming not supported by transport")
			return
		}
