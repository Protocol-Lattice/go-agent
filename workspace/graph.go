...
// belongs to the indexed Go module. External dependencies are omitted because
// they are not represented by source files in this index.
...
func (i *Index) Dependents(path string) []string {
	path = filepath.ToSlash(path)
	if _, ok := i.files[path]; !ok || i.module == "" {
		return nil
	}
